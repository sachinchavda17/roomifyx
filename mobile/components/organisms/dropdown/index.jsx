import React, { useState, createContext, useContext, useRef, Children, useCallback } from "react"
import { View, StyleSheet, Modal, TouchableOpacity } from "react-native"
import Animated, { useSharedValue, useAnimatedStyle, withTiming, interpolate, Easing, withSpring } from "react-native-reanimated"
...
const Trigger = ({ children, style }) => {
  ...
  return (
    <TouchableOpacity ref={triggerRef} onPress={handlePress} style={style} activeOpacity={0.7}>
      {children}
    </TouchableOpacity>
  )
}

const Content = ({ children, style, position = "auto" }) => {
  ...
  const triggerHaptic = useCallback(() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light), [])
  ...
  const calculatePosition = useCallback(() => {
    ...
    if (position === "auto") {
      ...
      if (spaceBelow >= contentHeight + SPACING) {
        ...
        top = spaceBelow > spaceAbove ? y + height + SPACING : Math.max(SPACING, y - contentHeight - SPACING)
      }
    ...
    // Final bounds check
    top = Math.max(SPACING, Math.min(top, SCREEN_HEIGHT - contentHeight - SPACING))
    left = Math.max(SPACING, Math.min(left, SCREEN_WIDTH - contentWidth - SPACING))
        ...
        <GestureDetector gesture={panGesture}>
          <Animated.View
            ...
            style={[
              ...
              style,
              {
                top: contentDimensions ? top : triggerLayout.y + triggerLayout.height + SPACING,
...
const Item = ({ children, onPress, style, index = 0 }) => {
