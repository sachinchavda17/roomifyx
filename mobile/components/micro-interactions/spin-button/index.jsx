...
import { CircularLoader } from "../../molecules/circular-loader"
...
const StaggeredText = ({ text, style, animationConfig, enterInitial, enterFinal, exitInitial, exitFinal }) => {
  const characters = Array.from(text)

  return (
    <Animated.View
      style={styles.textWrapper}
      layout={LinearTransition.duration(animationConfig.buttonTransitionDuration).easing(animationConfig.timing.easing)}
    >
...
const Character = ({ char, style, index, animationConfig, enterInitial, enterFinal, exitInitial, exitFinal }) => {
...
const SpinButton = ({
  ...
  const handlePress = useCallback(() => {
    ...
    buttonScale.value = withTiming(BUTTON_SCALE.pressed, {
      duration: mergedAnimationConfig.buttonPressDuration,
    })
    buttonScale.value = withTiming(BUTTON_SCALE.released, {
  ...
  const animatedButtonStyle = useAnimatedStyle(() => {
    const backgroundColor = interpolateColor(buttonBackgroundProgress.value, [0, 1], [mergedColors.idle.background, mergedColors.active.background])
    const activeOpacity = interpolate(buttonBackgroundProgress.value, [0, 1], [1, 0.72])
  ...
  const animatedTextStyle = useAnimatedStyle(() => {
    const color = interpolateColor(textColorProgress.value, [0, 1], [mergedColors.idle.text, mergedColors.active.text])
  ...
  const animatedSpinnerContainerStyle = useAnimatedStyle(() => {
    return {
      ...
      backgroundColor: interpolateColor(spinnerScale.value, [0, 1], ["transparent", mergedSpinnerConfig.containerBackground]),
    }
  })
  ...
  return (
    <Pressable onPress={handlePress} disabled={disabled}>
      <Animated.View
        ...
        layout={LinearTransition.duration(mergedAnimationConfig.buttonTransitionDuration).easing(mergedAnimationConfig.timing.easing)}
      >
        <StaggeredText
