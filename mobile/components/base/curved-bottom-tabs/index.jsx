  ...
  return (
    ...
    >
      <Svg width={buttonSize} height={buttonSize}>
        <Defs>
          <SvgGradient id="floatingButtonGradient" x1="0%" y1="25%" x2="80%" y2="100%">
            <Stop offset="0%" stopColor={gradient[0]} />
            <Stop offset="100%" stopColor={gradient[1]} />
          </SvgGradient>
        </Defs>
        <Circle cx={buttonSize / 2} cy={buttonSize / 2} r={buttonSize / 2.2} fill="url(#floatingButtonGradient)" />
      </Svg>
      <View
        style={{
          position: "absolute",
...
const BackgroundCurve = memo(({ position, gradient, height }) => {
  ...
  return (
    <View style={{ overflow: "hidden", width: screenWidth }}>
      <Animated.View style={animatedStyle}>
        <Svg width={totalWidth} height={height} preserveAspectRatio="none" viewBox={`0 0 ${totalWidth} ${height}`} fill="none">
          <Path d={path} fill="url(#curveGradient)" />
          <Defs>
            <SvgGradient id="curveGradient" x1={0} y1={height * 0.8} x2={totalWidth} y2={height * 0.8} gradientUnits="userSpaceOnUse">
...
const CurvedBottomTabsCore = memo(
  ({
    tabs,
    currentIndex,
    onPress,
    ...
    const curvePosition = useSharedValue(0)
    const floatingAnimations = useRef(tabs.map(() => useSharedValue(0))).current

    const processedGradient = processGradient(gradient)
    ...
    const animateToIndex = (targetIndex) => {
      const targetPosition = calculateTabPosition(targetIndex, tabs.length)

      curvePosition.value = withSpring(targetPosition, {
      ...
      floatingAnimations.forEach((anim, index) => {
        anim.value = withSpring(index === targetIndex ? -VIEWPORT_HEIGHT * 4.2 : 0, {
          damping: 10,
          stiffness: 100,
          mass: 0.5,
        })
    ...
    return (
      <View style={styles.wrapper}>
        <View style={styles.backgroundContainer}>
          <BackgroundCurve position={curvePosition} gradient={processedGradient} height={Math.ceil(VIEWPORT_HEIGHT * barHeight)} />
        </View>
        ...
        {tabs.map((tab, index) => {
          ...
          return (
            <Animated.View key={tab.id} style={[styles.tabWrapper, animatedTabStyle]}>
              <TouchableOpacity onPress={handlePress} style={styles.tabTouchable} activeOpacity={0.9}>
                {isActive ? (
