...
import Animated, { useSharedValue, useAnimatedStyle, withRepeat, withTiming, interpolate, Easing, LinearTransition } from "react-native-reanimated"
...
export const Avatar = memo(
  ({
    ...
    const shimmerAnimatedStylez = useAnimatedStyle(() => {
      const translateX = interpolate(shimmerProgress.value, [0, 1], [-size * 1.5, size * 1.5])
      const opacity = interpolate(shimmerProgress.value, [0, 0.5, 1], [0.3, 0.8, 0.3])
    ...
    const fadeAnimatedStylez = useAnimatedStyle(() => ({
    ...
    const AvatarContent = () => {
      if (loading) {
        return (
          <Animated.View layout={LinearTransition.springify()}>
            <View style={[styles.loadingContainer, avatarSize, { backgroundColor: getBackgroundColor() }]}>
              <Text style={[styles.fallbackText, { fontSize, color: textColor, opacity: 0.6 }]} numberOfLines={1}>
      ...
      if (shouldShowImage) {
        return (
          <Animated.View style={[styles.imageContainer, fadeAnimatedStylez]}>
            <Image source={{ uri: image.uri }} style={[styles.avatar, avatarSize, showBorder && { borderWidth, borderColor }]} />
            {showOnlineIndicator && (
              <View
                style={[
      ...
      return (
        <Animated.View style={[styles.fallbackContainer, fadeAnimatedStylez]}>
          <View style={[styles.fallback, avatarSize, { backgroundColor: getBackgroundColor() }, showBorder && { borderWidth, borderColor }]}>
            <Text style={[styles.fallbackText, { fontSize, color: textColor }]} numberOfLines={1}>
              {getInitials(image?.name)}
            </Text>
          </View>
        </Animated.View>
    ...
    return (
      <View style={[styles.container, textPosition === "right" && styles.containerRow, textPosition === "top" && styles.containerColumn]}>
        ...
        <AnimatedPressable
          ...
          style={[styles.pressable, pressAnimatedStylez, disabled && styles.disabled]}
        >
          <AvatarContent />
        </AnimatedPressable>
